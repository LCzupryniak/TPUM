...
namespace Client.Logic.Implementation
{
    internal class CartLogic : ICartLogic
    {
        ...
        public static IInventoryDataTransferObject Map(ICart inventory)
        {
            ...
            return new CartDataTransferObject(inventory.Id, inventory.Capacity, mappedItems);
        }
        ...
        public void Add(IInventoryDataTransferObject inventory)
        {
            lock (_lock)
            {
                _repository.AddInventory(new MappedDataCart(inventory));
            }
        }
        ...
        public bool Remove(IInventoryDataTransferObject inventory)
        {
            lock (_lock)
            {
                return _repository.RemoveInventory(new MappedDataCart(inventory));
            }
        }
        ...
        public bool Update(Guid id, IInventoryDataTransferObject inventory)
        {
            lock (_lock)
            {
                return _repository.UpdateInventory(id, new MappedDataCart(inventory));
            }
        }
    }
}
